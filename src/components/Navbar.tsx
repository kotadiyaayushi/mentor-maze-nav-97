...
const Navbar = ({ onToggleSidebar, showSidebarToggle = false, title = "SkillSphere", className = '' }: NavbarProps) => {
        ...
        <div className="relative">
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-lg shadow-lg z-50">
              <div className="py-2">
                <button className="w-full text-left px-4 py-2 hover:bg-surface transition-colors">
                  Profile Settings
                </button>
                <button 
                  onClick={handleLogout}
                  className="w-full text-left px-4 py-2 hover:bg-surface transition-colors flex items-center space-x-2 text-error"
                >
                  <LogOut size={16} />
                  <span>Logout</span>
                </button>
